# `create` — set up hiker intent for an initiative
...
3. **Write `.hiker/tents/<slug>/<slug>.tent`.** Declare:
...
4. **Write `.hiker/tents/<slug>/CONTEXT.md`** — the prose the spec can't hold:
...
5. **Compile it:** `hiker check .hiker/tents/<slug>/<slug>.tent` until it
...
```
// .hiker/tents/typed-errors/typed-errors.tent
...
```sh
hiker check .hiker/tents/typed-errors/typed-errors.tent
# OK: 3 sorts, 2 relations, 2 laws
```

## Done when

- `.hiker/tents/<slug>/<slug>.tent` compiles (`OK: ...`).
- `.hiker/tents/<slug>/CONTEXT.md` records the invariant + boundary.
- A `check` script exists and `.hiker-cache/` is gitignored.
